		...
		float h)
{
	...
	// Drop shadow
	shadowPaint =
		nvgBoxGradient(vg, x, y + 2, w, h, cornerRadius * 2, 10,
		   ...
		   float h)
{
	...
	// Edit
	bg = nvgBoxGradient(vg, x, y + 1.5f, w, h, h / 2, 5,
		  ...
		  float h)
{
	...
	nvgBeginPath(vg);
	nvgRoundedRect(vg, x + 0.5f, y + 0.5f, w - 1, h - 1,
		       cornerRadius - 0.5f);
	nvgStrokeColor(vg, nvgRGBA(0, 0, 0, 48));
	nvgStroke(vg);
		  ...
		  float h)
{
	...
	bg = nvgBoxGradient(vg, x + 1, y + (int)(h * 0.5f) - 9 + 1, 18, 18, 3,
			    3, nvgRGBA(0, 0, 0, 32), nvgRGBA(0, 0, 0, 92));
	nvgBeginPath(vg);
		...
		float w, float h, NVGcolor col)
{
	...
	if (preicon != 0) {
		nvgFontSize(vg, h * 1.3f);
		nvgFontFace(vg, "icons");
		iw = nvgTextBounds(vg, 0, 0, cpToUTF8(preicon, icon), NULL,
				   NULL);
		iw += h * 0.15f;
	}
	...
	nvgText(vg, x + w * 0.5f - tw * 0.5f + iw * 0.25f, y + h * 0.5f - 1,
		text, NULL);
	nvgFillColor(vg, nvgRGBA(255, 255, 255, 160));
	      ...
	      float my, float t)
{
	...
	dy *= ey * 0.5f;
	nvgBeginPath(vg);
	nvgEllipse(vg, lx + dx, ly + dy + ey * 0.25f * (1 - blink), br,
		   br * blink);
	...
	dy *= ey * 0.5f;
	nvgBeginPath(vg);
	nvgEllipse(vg, rx + dx, ry + dy + ey * 0.25f * (1 - blink), br,
		   br * blink);
	nvgFillColor(vg, nvgRGBA(32, 32, 32, 255));
	nvgFill(vg);

	gloss = nvgRadialGradient(vg, lx - ex * 0.25f, ly - ey * 0.5f,
				  ex * 0.1f, ex * 0.75f,
		    ...
		    const int *images, int nimages, float t)
{
	...
	// Drop shadow
	shadowPaint =
		nvgBoxGradient(vg, x, y + 4, w, h, cornerRadius * 2, 20,
			       nvgRGBA(0, 0, 0, 128), nvgRGBA(0, 0, 0, 0));
	nvgBeginPath(vg);
	nvgRect(vg, x - 10, y - 10, w + 20, h + 30);
	nvgRoundedRect(vg, x, y, w, h, cornerRadius);
	...
	for (i = 0; i < nimages; i++) {
		...
		if (a < 1.0f)
			drawSpinner(vg, tx + thumb / 2, ty + thumb / 2,
				    thumb * 0.25f, t);

		imgPaint =
			nvgImagePattern(vg, tx + ix, ty + iy, iw, ih,
		...
		shadowPaint =
			nvgBoxGradient(vg, tx - 1, ty, thumb + 2, thumb + 2, 5,
				       3, nvgRGBA(0, 0, 0, 128),
				       nvgRGBA(0, 0, 0, 0));
		nvgBeginPath(vg);
		...
		nvgBeginPath(vg);
		nvgRoundedRect(vg, tx + 0.5f, ty + 0.5f, thumb - 1, thumb - 1,
			       4 - 0.5f);
		nvgStrokeWidth(vg, 1.0f);
	...
	// Hide fades
	fadePaint = nvgLinearGradient(vg, x, y, x, y + 6,
	...
	fadePaint = nvgLinearGradient(vg, x, y + h, x, y + h - 6,
				      nvgRGBA(200, 200, 200, 255),
				      nvgRGBA(200, 200, 200, 0));
	nvgBeginPath(vg);
	...
	// Scroll bar
	shadowPaint =
		nvgBoxGradient(vg, x + w - 12 + 1, y + 4 + 1, 8, h - 8, 3, 4,
	...
	shadowPaint = nvgBoxGradient(vg, x + w - 12 - 1,
				     y + 4 + (h - 8 - scrollh) * u - 1, 8,
				     scrollh, 3, 4, nvgRGBA(220, 220, 220, 255),
				     nvgRGBA(128, 128, 128, 255));
	nvgBeginPath(vg);
	nvgRoundedRect(vg, x + w - 12 + 1, y + 4 + 1 + (h - 8 - scrollh) * u,
		       8 - 2, scrollh - 2, 2);
	nvgFillPaint(vg, shadowPaint);
	//	nvgFillColor(vg, nvgRGBA(0,0,0,128));
	nvgFill(vg);
...
void drawColorwheel(NVGcontext *vg, float x, float y, float w, float h, float t)
{
	...
	r0 = r1 - 20.0f;
	aeps = 0.5f /
	       r1; // half a pixel arc length in radians (2pi cancels out).
	...
	nvgLineTo(vg, bx, by);
	nvgClosePath(vg);
	paint = nvgLinearGradient(vg, r, 0, ax, ay,
...
int loadDemoData(NVGcontext *vg, DemoData *data)
{
	...
	for (i = 0; i < 12; i++) {
		char file[128];
		snprintf(file, 128, "res/images/image%d.jpg", i + 1);
	...
	data->fontIcons = nvgCreateFont(vg, "icons", "res/fonts/entypo.ttf");
	if (data->fontIcons == -1) {
	...
	data->fontEmoji =
		nvgCreateFont(vg, "emoji", "res/fonts/NotoEmoji-Regular.ttf");
	if (data->fontEmoji == -1) {
		   ...
		   float mx, float my)
{
	...
	while ((nrows = nvgTextBreakLines(vg, start, end, width, rows, 3))) {
		for (i = 0; i < nrows; i++) {
			...
			if (hit) {
				caretx = (mx < x + row->width / 2) ?
						 x :
						 x + row->width;
				px = x;
				nglyphs = nvgTextGlyphPositions(vg, x, y,
								row->start,
								row->end,
								glyphs, 100);
				for (j = 0; j < nglyphs; j++) {
					float x0 = glyphs[j].x;
					float x1 = (j + 1 < nglyphs) ?
							   glyphs[j + 1].x :
							   x + row->width;
	...
	gx = fabsf((mx - (bounds[0] + bounds[2]) * 0.5f) /
		   (bounds[0] - bounds[2]));
	gy = fabsf((my - (bounds[1] + bounds[3]) * 0.5f) /
		   (bounds[1] - bounds[3]));
	a = maxf(gx, gy) - 0.5f;
	a = clampf(a, 0, 1);
	nvgGlobalAlpha(vg, a);
		...
		float t, int blowup, DemoData *data)
{
	...
	drawButton(vg, ICON_TRASH, "Delete", x, y, 160, 28,
		   nvgRGBA(128, 16, 8, 255));
